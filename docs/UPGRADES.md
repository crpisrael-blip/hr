# שדרוגי תלויות פתוחים

נכון ל-11.9.2026. `npm audit --omit=dev` מדווח על **5 פגיעויות** — 1 critical,
1 high, 2 moderate, 1 low. **כולן** נסגרות רק בשני שדרוגי major:
`astro@7` ו-`react-router-dom@7`. אין תיקון patch זמין לאף אחת מהן.

השדרוגים **לא בוצעו**: הם דורשים התקנה, בנייה ובדיקה ידנית של שלוש
האפליקציות, וזו החלטה של בעל הריפו. עד שיבוצעו, הצעד `npm audit` ב-CI מסומן
`continue-on-error` (ראו `.github/workflows/ci.yml`) — יש להסיר את הסימון
מיד עם סיום השדרוגים, אחרת הצעד לעולם לא יחסום.
